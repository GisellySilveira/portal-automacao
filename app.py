...
try:
    ...
    from config_hardcoded import get_api_config, is_configured
    print("[OK] Modulos de API importados com sucesso")
except ImportError as e:
    st.warning(f"Módulo de integração com API não encontrado: {e}")
    APIShipSmart = None
...
def enviar_para_api(arquivos_gerados, transportadora, nome_cliente, modalidade=""):
    ...
    print("\n" + "="*80)
    print("[DEBUG] Funcao enviar_para_api foi chamada!")
    print(f"[DEBUG] Arquivos: {len(arquivos_gerados)}")
    print(f"[DEBUG] Transportadora: {transportadora}")
    print(f"[DEBUG] Cliente: {nome_cliente}")
    print(f"[DEBUG] Modalidade: {modalidade}")
    print("="*80 + "\n")
    
    try:
        # Usa configurações hardcoded
        from config_hardcoded import get_api_config
        config = get_api_config()
        
        print("[DEBUG] Configuracoes carregadas")
        print(f"[DEBUG] Token configurado: {'Sim' if config.get('token') else 'Nao'}")
        print(f"[DEBUG] API URL: {config.get('api_url')}")
        
        if not config.get("token"):
            st.error("❌ Token não configurado em config_hardcoded.py")
            return None
        
        # Inicializa cliente da API
        print("[DEBUG] Inicializando cliente da API...")
        api = APIShipSmart(
            base_url=config.get("api_url", "https://bck.shipsmart.com.br/api"),
            token=config.get("token")
        )
        print("[DEBUG] Cliente da API inicializado!")
        
        # Envia arquivos com formato correto
        with st.spinner(f"📤 Enviando {len(arquivos_gerados)} arquivo(s) para o sistema..."):
            print("[DEBUG] Chamando api.enviar_multiplas_tabelas...")
            ...
            print(f"[DEBUG] Retornou {len(resultados) if resultados else 0} resultados")
        
        return resultados
        
    except Exception as e:
        print(f"[DEBUG ERRO] Excecao: {e}")
        import traceback
        print(traceback.format_exc())
        st.error(f"❌ Erro ao enviar para API: {e}")
        return None
...
if escolha_topico == "Página Inicial":
    ...
    # Tabs para Exportação e Importação
    tab_exportacao, tab_importacao = st.tabs(["📤 Exportação", "📥 Importação"])
    
    # ========================================
    # TAB EXPORTAÇÃO
    # ========================================
    with tab_exportacao:
        st.write("Selecione uma das opções abaixo para processar tabelas de exportação.")
        ...
        with st.expander("✈️ FedEx", expanded=True):
            ...
            with col_uploader_fedex:
                ...
                with col_margem:
                    adicionar_margem_fedex = st.checkbox("📊 Gerar margem", value=True, key="margem_fedex")
                ...
                with st.expander("⚙️ Configurações Avançadas"):
                    ...
                    with col_priority:
                        processar_priority = st.checkbox("🚀 Priority", value=True, key="proc_priority_fedex")
                            ...
                            with st.spinner("Aguarde... Processando a planilha FedEx..."):
                                try:
                                    ...
                                    if not servicos_processar:
                                        ...
                                        if arquivos_gerados and len(arquivos_gerados) > 0:
                                            ...
                                            with col_api:
                                                if st.button("🚀 Enviar para o Sistema", key="enviar_api_fedex", use_container_width=True):
                                                    print("\n" + "="*80)
                                                    print("[BOTAO CLICADO] Usuario clicou em 'Enviar para o Sistema'")
                                                    print("="*80 + "\n")
                                                    ...
                                                    print(f"[DEBUG] Modalidade identificada: {modalidade_fedex}")
                                                    print(f"[DEBUG] Arquivos a enviar: {len(arquivos_gerados)}")
        ...
        with st.expander("📦 UPS"):
            ...
            with col_uploader_ups:
                ...
                with col_margem:
                    adicionar_margem_ups = st.checkbox("📊 Gerar margem", value=True, key="margem_ups")
                ...
                with st.expander("⚙️ Configurações Avançadas"):
                    st.markdown("### 📊 Selecionar Serviços para Processar")
                    st.info("Escolha quais serviços da UPS você deseja processar:")
                    
                    col_saver, col_expedited, col_ground = st.columns(3)
                    
                    with col_saver:
                        processar_saver = st.checkbox("📦 Saver", value=True, key="proc_saver_ups")
                    
                    with col_expedited:
                        processar_expedited = st.checkbox("⚡ Expedited", value=True, key="proc_expedited_ups")
                    
                    with col_ground:
                        processar_ground = st.checkbox("🚚 Ground", value=True, key="proc_ground_ups")
                        ...
                        if not nome_cliente_ups or nome_cliente_ups.strip() == "":
                            ...
                            with st.spinner("Aguarde... Processando a planilha UPS..."):
                                try:
                                    ...
                                    servicos_processar = []
                                    if processar_saver:
                                        servicos_processar.append('Saver')
                                    if processar_expedited:
                                        servicos_processar.append('Expedited')
                                    if processar_ground:
                                        servicos_processar.append('Ground')
                                        ...
                                        if arquivos_gerados:
                                            ...
                                            with col_api:
                                                if st.button("🚀 Enviar para o Sistema", key="enviar_api_ups", use_container_width=True):
                                                    # Identifica modalidade predominante dos serviços selecionados
                                                    modalidade_ups = ""
                                                    if processar_saver and not processar_expedited and not processar_ground:
                                                        modalidade_ups = "Saver"
                                                    elif processar_expedited and not processar_saver and not processar_ground:
                                                        modalidade_ups = "Expedited"
                                                    elif processar_ground and not processar_saver and not processar_expedited:
                                                        modalidade_ups = "Ground"
                                                    else:
                                                        modalidade_ups = "Multi"
                                                    
                                                    resultados = enviar_para_api(
                                                        arquivos_gerados,
                                                        'UPS',
                                                        nome_cliente_ups.strip(),
                                                        modalidade=modalidade_ups
                                                    )
        ...
        with st.expander("🚚 DHL"):
            ...
            with col_uploader_dhl:
                ...
                with col_margem:
                    adicionar_margem_dhl = st.checkbox("📊 Gerar margem", value=True, key="margem_dhl")
        ...
        with st.expander("📮 Outras Transportadoras"):
            ...
            with col_margem:
                adicionar_margem_outras = st.checkbox("📊 Gerar margem", value=True, key="margem_outras")
            ...
            # Upload do arquivo
            arquivo_excel_outras = st.file_uploader("Escolha a planilha da transportadora", type=["xlsx", "xls"], key="outras_excel")
    ...
    with tab_importacao:
        ...
        with col_servico:
            if transportadora_import == "FEDEX":
                ...
                servicos_opcoes = ["Saver", "Expedited", "Ground", "Todos"]
